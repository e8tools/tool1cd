...
uint32_t CommonFieldType::getSortKey(const char* rec, unsigned char* SortKey, int32_t maxlen) const
{
	...
	if (!maxlen) {
		throw SerializationException("Ошибка получения ключа сортировки поля. Нулевая длина буфера.")
				.add_detail("Значение поля", get_presentation(rec));
	}
...
FieldType *FieldType::parse_field_type(tree *field_tree)
{
	...
	if     (sFieldType == "B")   type = type_fields::tf_binary;
	else if(sFieldType == "L")   type = type_fields::tf_bool;
	else if(sFieldType == "N")   type = type_fields::tf_numeric;
	else if(sFieldType == "NC")  type = type_fields::tf_char;
	else if(sFieldType == "NVC") type = type_fields::tf_varchar;
	else if(sFieldType == "RV")  type = type_fields::tf_version;
	else if(sFieldType == "NT")  type = type_fields::tf_string;
	else if(sFieldType == "T")   type = type_fields::tf_text;
	else if(sFieldType == "I")   type = type_fields::tf_image;
	else if(sFieldType == "DT")  type = type_fields::tf_datetime;
	else if(sFieldType == "VB")  type = type_fields::tf_varbinary;
	else {
	...
	if     (sNullExists == "0") null_exists = false;
	else if(sNullExists == "1") null_exists = true;
	else {
	...
	if     (sCaseSensitive == "CS") case_sensitive = true;
	else if(sCaseSensitive == "CI") case_sensitive = false;
	else {
	...
	CommonFieldType *result = new CommonFieldType();
	result->case_sensitive = case_sensitive;
	result->precision = precision;
	result->length = length;
	result->null_exists = null_exists;
	result->type = type;
...
{
	CommonFieldType *result = new CommonFieldType();
	result->type = type_fields::tf_version8;
	return result;
}