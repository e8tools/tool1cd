...
Программа работает с файлами `1CD` по версии формата, записанной в заголовке базы:

| Версия формата | Режим |
|---|---|
| 8.0.3, 8.0.5 | только чтение |
| 8.1.0, 8.2.0, 8.2.14, 8.3.8 | чтение и запись |

Формат `8.3.8` используют все платформы 1С:Предприятие 8.3.8 и новее (в том числе 8.3.x и 8.5.x) — их базы открываются как `8.3.8`. Версия формата в заголовке не обязательно совпадает с версией платформы, создавшей базу. Базы с другой версией формата не поддерживаются.
 ...
 * [Win-x64 консольное](https://storage.yandexcloud.net/appveyor/tool1cd/develop/win/ctool1cd.exe)
 * [Win-x64 GUI](https://storage.yandexcloud.net/appveyor/tool1cd/develop/win/tool1cd.zip)
...
## Использование
 ...
 `-sts, -SaveTablesSize <путь>`
   ...
   Сохранить размеры таблиц базы в указанный файл.

 `-ct, -ClearTable <таблица>`

   Пометить удалёнными все записи указанной таблицы. Команда изменяет базу.

 `-dr, -DeleteRecord <таблица> <номер>`

   Пометить удалённой одну запись таблицы по её физическому номеру. Команда изменяет базу.
